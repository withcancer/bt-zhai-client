    ...
    <section class="result-c">
      ...
      <div class="right-c">
          ...
          <div v-if="pager.total" class="footer-c">
            <no-ssr>
              <pagination :total="pager.total > pager.overTotal ? pager.overTotal / 10: (pager.total / 10) + 1"
...
export default {
  ...
  async asyncData({ app, params }) {
    ...
    return app.$axios
      ...
      .then(res => {
        let { data, code, total, took, msg } = res;
        if (code != 200) {
  ...
  methods: {
    ...
    onClickName(id) {
      let torrent = this.data.find(item => item.id == id);
